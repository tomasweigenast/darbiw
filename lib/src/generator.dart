...
final class BinaryGenerator extends GeneratorForAnnotation<Binary> {
  ...
  FutureOr<String> generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) {
    ...
    final constructor = element.constructors.where((element) => element.name.isEmpty).firstOrNull;
    if (constructor == null) {
      print("ignoring ${element.name} because not default constructor is found.");
      return "";
    }
    ...
    for (final parameter in constructor.parameters) {
      final (writer, size) = _getNullabilityWriter(parameter.name, parameter.type);
      bufferSize += size;
      encoders.add(writer);
    ...
    encoderBuffer.write("return writer.takeBytes(); }}"); // end of toBuffer. end of extension declx

    // write XFromBuffer method
    decoderBuffer.write('@pragma("vm:prefer-inline")');
    decoderBuffer.write("""${element.name} _${element.name}FromBuffer(Uint8List buffer) {
      final reader = BinaryReader(buffer);
      return ${element.name}(""");
    decoderBuffer.writeAll(decoders);
    decoderBuffer.write("); }"); // end of constructor - end of XFromBuffer method

    return encoderBuffer.toString() + decoderBuffer.toString();
  }
}

(String writer, int bufferSize) _getNullabilityWriter(String paramName, DartType type) {
...
(String writer, int bufferSize) _getWriter(String paramName, DartType type, [bool isNullable = false]) {
  if (type.isDartCoreBool) {
    ...
    return ("$paramName is int ? writer.writeInt($paramName) : writer.writeDouble($paramName);", 8);
    ...
    final baseTypeIsNullable = baseType.nullabilitySuffix == NullabilitySuffix.question;
    var (baseTypeWriter, size) = _getWriter("$paramName${isNullable ? '!' : ''}[i]${baseTypeIsNullable ? '!' : ''}", baseType);
    if (baseTypeIsNullable) {
      baseTypeWriter = "$paramName${isNullable ? '!' : ''}[i] == null ? writer.writeNull() : $baseTypeWriter";
    }
...
""",
    ...
    if (type.element is EnumElement) {
      ...
      return ("writer.writeUint8List($paramName${isNullable ? '!' : ''}.toBuffer());", 512);
    }
  }
